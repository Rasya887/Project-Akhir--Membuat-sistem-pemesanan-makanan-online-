...
if (isset($_POST['terima_orderitem_validate']) && !empty($id_list_order)) {
    ...
    if ($catatan === $order_data['catatan']) {
        ...
        $update_query = mysqli_query($conn, "UPDATE tb_list_order SET catatan='$catatan', status=1 WHERE id_list_order='$id_list_order'");
        
        if ($update_query) {
            echo '<script>alert("Berhasil menerima order!"); window.location="../kitchen.php?x=kitchen";</script>';
        } else {
            echo '<script>alert("Gagal menerima order: ' . mysqli_error($conn) . '"); window.location="../kitchen.php?x=kitchen";</script>';
        }
    } else {
